export function logout() {
    ...
    //logout_ID
    const agencylogout = document.getElementById('logout_ID');
    if (agencylogout) {
        agencylogout.addEventListener('click', function (event) {
            event.preventDefault();
            console.log('Logout');
            if (confirm('Are you sure you want to logout?')) {
                console.log('Logout');
                // Remove 'adminSessionData' and 'adminUserID' from localStorage
                // if (localStorage.getItem('adminSessionData') !== null && localStorage.getItem('adminUserID') !== null) {
                localStorage.removeItem('agencySessionData');
                localStorage.removeItem('agencyUserID');
                // }
