...
export function agencyProfleFunctions() {
    ...
    const profileElement = document.getElementById('agencyImageInput');
    if (profileElement) {
    ...
    let dataCollection = [];
    let currentData = [];
    const agencyProfileForm = document.getElementById('editAgencyProfile');
    if (agencyProfileForm) {
        agencyProfileForm.addEventListener('submit', async function (event) {
            event.preventDefault();
            if (confirm('Are you sure you want to edit the hashed password?')) {

                const updatedAgencyName = document.getElementById('inputName').value;
                ...
                const imgElement = document.getElementById('agencyProfileImage');
                // Get the value of the src attribute
                const srcValue = imgElement.getAttribute('src');
                //Get the current data in session
                getSessionData();

                if (currentData[1] == updatedAgencyName && currentData[2] == updatedEmail && currentData[3] == updatedPassword && currentData[4] == srcValue) {
                    ...
                    if (updatedEmail !== currentData[2]) {
                        ...
                        if (currentPassword === updatedPassword) {
                            dataCollection = [currentData[0], updatedAgencyName, updatedEmail, updatedPassword, adminProfile];
                        } else {
                            const hashedUserPassword = await hashPassword(updatedPassword);
                            dataCollection = [currentData[0], updatedAgencyName, updatedEmail, hashedUserPassword, adminProfile];
                        }

                    } else {
                        saveAdmin([currentData[0], updatedAgencyName, updatedEmail, updatedPassword, srcValue]);
                    }
    ...
    async function saveAdmin(updatedData) {
        ...
        const collectionRefSub = collection(db, "Subscribers");
        const queryRefSub = query(collectionRefSub, where('agency_id', '==', id));

        const newData = {
            companyNameName: updatedData[1],
            agecny_email: updatedData[2],
            agecny_password: updatedData[3],
            agecny_profile: updatedData[4],
        }
        console.log(newData);
        const querySnapshot = await getDocs(queryRef);
        const querySnapshot2 = await getDocs(queryRefSub);
        ...
        if (!querySnapshot2.empty) {
            ...
            // Update Firestore document
            await setDoc(agencyDocRef, subData, { merge: true });
        }
    ...
    function getSessionData() {
        ...
        if (agencySessionData !== null) {
            ...
            for (const agencyData of data) {
                currentData.push(agencyData.userId);
                currentData.push(agencyData.companyName);
                currentData.push(agencyData.agency_email);
                currentData.push(agencyData.agencyPassword);
                currentData.push(agencyData.profileImage);
            }
        }
    }
    ...
    function displayUpdatedSessionData(updatedData) {
        ...
        if (agencySessionData !== null) {
            ...
            for (const agencyData of data) {
                ...
                agencyData.profileImage = updatedData[4]; // Update the profile image path
                agencyData.companyName = updatedData[1]; // Update the admin name
                agencyData.agency_email = updatedData[2]; // Update the admin email
                agencyData.agencyPassword = updatedData[3]; // Update the admin password
                agencyData.userId = updatedData[0]; // Update the user ID
            }

            // Store the modified data back in localStorage
            localStorage.setItem('agencySessionData', JSON.stringify(data));

            document.getElementById('agencyAvatarID').src = updatedData[4];
            document.getElementById('agencyProfileImage').src = updatedData[4];
